...
public final class SocketChannelProvider<T extends Channel> implements ChannelFactory<T> {
    ...
    public T newChannel() {
        return switch (channelType) {
            case ACCEPTOR -> switch (socketType) {
                case JAVA_NIO -> (T) new NioServerSocketChannel();
                case NATIVE_EPOLL -> (T) new EpollServerSocketChannel();
                case NATIVE_KQUEUE -> (T) new KQueueServerSocketChannel();
                case NATIVE_EPOLL_DOMAIN -> (T) new EpollServerDomainSocketChannel();
                case NATIVE_KQUEUE_DOMAIN -> (T) new KQueueServerDomainSocketChannel();
            };
            case CONNECTOR -> switch (socketType) {
                case JAVA_NIO -> (T) new NioSocketChannel();
                case NATIVE_EPOLL -> (T) new EpollSocketChannel();
                case NATIVE_KQUEUE -> (T) new KQueueSocketChannel();
                case NATIVE_EPOLL_DOMAIN -> (T) new EpollDomainSocketChannel();
                case NATIVE_KQUEUE_DOMAIN -> (T) new KQueueDomainSocketChannel();
            };
        };
    }
